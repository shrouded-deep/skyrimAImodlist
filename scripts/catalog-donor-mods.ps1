...
# Report-only ΓÇö does not copy or rename folders.
...
function Get-CleanModName([string]$RawName) {
    ...
    # List tier prefix: 005.006 Γùë (U+25C9 FISHEYE)
...
$stats = @{
    ...
    ImportCandidates = @($rows | Where-Object {
        # Nexus rows selectable in the picker — including mods already present in LL
        # (no de-dupe). Separators are listed separately as section markers.
        $_.Resolution -eq 'nexus' -and -not $_.ExcludeFromImport
    }).Count
...
function Table-Rows($items, [string]$RowClass) {
    ...
    foreach ($r in $items) {
        ...
        $link = if ($r.NexusUrl) { "<a href=`"$($r.NexusUrl)`" target=`"_blank`">$($r.NexusUrl)</a>" } else { 'ΓÇö' }
        $modId = if ($r.ModId) { $r.ModId } else { 'ΓÇö' }
        $excl = if ($r.ExcludeReason) { Escape-Html $r.ExcludeReason } else { 'ΓÇö' }
...
$html += '<p>LoversLab, Patreon, manual/local installs, tool outputs, list-only presets ΓÇö not candidates for Lost Legacy Fork.</p>'
$html += (Table-Rows $excludedRows 'nometa')

$html += "<h2 id=`"candidates`">Nexus import candidates ($($candidateRows.Count))</h2>"
$html += '<p>Nexus modid resolved (includes LL duplicates — picker does not de-dupe).</p>'
...
$pickerRows = @($rows | Where-Object {
    $_.IsSeparator -or ($_.Resolution -eq 'nexus' -and -not $_.ExcludeFromImport)
...
"@)
    ...
    [void]$pickerBody.AppendLine(@"
<tr data-tier="$(Escape-Html $r.ListTier)"$(if ($r.DuplicateInLL) { ' class="dup"' } else { '' })>
  <td><input type="checkbox" class="pick" data-folder="$(Escape-Html $r.RawFolder)" id="pick-$fid"/></td>
  <td class="tier">$(Escape-Html $r.ListTier)</td>
  <td><label for="pick-$fid">$(Escape-Html $r.RawFolder)</label>$dup</td>
  <td>$link</td>
...
tr:nth-child(even) { background: #222228; }
tr.sep { background: #2a3340; color: #c9a227; }
tr.sep td { border-color: #3a5060; }
tr.dup { background: #2a2218; }
